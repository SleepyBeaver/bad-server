...
// Utility: безопасный RegExp для поиска
    ...
    try {
        ...
        if (search && typeof search === 'string') {
            ...
            orders = orders.filter((order) => {
                const matchesProduct = order.products.some((p) => productIds.some((id) => (p._id as Types.ObjectId).equals(id)))
        ...
        res.status(200).json({ orders, pagination: { totalOrders, totalPages, currentPage: Number(page), pageSize: Number(limit) } })
    } catch (error) {
        next(error)
    }
}
...
export const updateOrder = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        ).populate(['customer', 'products']).orFail(() => new NotFoundError('Заказ не найден'))
